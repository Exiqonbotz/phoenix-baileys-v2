...
A high-performance WhatsApp Web library built on [Baileys](https://github.com/WhiskeySockets/Baileys), with critical paths accelerated via a [Rust WASM bridge](https://github.com/Exiqonbotz/whatsapp-rust-bridge).
...
// Username lookup
const user = await sock.findUserByUsername('Exiqon')
...
```bash
npm install github:Exiqonbotz/phoenix-baileys-v2
# or
...
```js
const { makeWASocket, useMultiFileAuthState, DisconnectReason } = require('phoenix-baileys-v2')
...
```js
const { useMultiFileAuthState } = require('phoenix-baileys-v2')
...
```js
const { getAggregateVotesInPollMessage } = require('phoenix-baileys-v2')
...
Import from `phoenix-baileys-v2/src/antiban.js`:
...
} = require('phoenix-baileys-v2/src/antiban')
```
...
```js
const { wrapSocket, resolveConfig, PRESETS } = require('phoenix-baileys-v2/src/antiban')
...
```js
const { downloadMediaMessage } = require('phoenix-baileys-v2')
...
The native module lives at [Exiqonbotz/whatsapp-rust-bridge](https://github.com/Exiqonbotz/whatsapp-rust-bridge).  
