...
Baileys and its maintainers cannot be held liable for misuse of this application, as stated in the [MIT license](https://github.com/Exiqonbotz/phoenix-baileys-v2/LICENSE).
...
```ts
import makeWASocket, { useSingleFileAuthState, useMongoFileAuthState } from 'phoenix-baileys-v2'
...
const connectAuth = async() => {
  ...
  const collection = client.db("@Team-Phoenix").collection("sessions")
  return collection
}
...
const logger = pino({ timestamp: () => `,"time":"${new Date().toJSON()}"` }).child({ class: "@Team-Phoenix" })
logger.level = "fatal"
...
import { downloadMediaMessage, getContentType } from 'phoenix-baileys-v2'
