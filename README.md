...
const connectAuth = async() => {
  ...
  const collection = client.db("@Exiqon").collection("sessions")
  return collection
}
...
const logger = pino({ timestamp: () => `,"time":"${new Date().toJSON()}"` }).child({ class: "@Exiqon" })
logger.level = "fatal"
