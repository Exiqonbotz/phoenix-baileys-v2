...
phoenix-baileys-v2 exposes all MEX operations as first-class socket methods. MEX is WhatsApp's internal GraphQL system that runs over the existing WebSocket connection — no extra HTTP requests, no authentication tokens needed beyond your normal session.
...
```js
const { makeWASocket, useMultiFileAuthState } = require('phoenix-baileys-v2')
