...
The antiban system is based on [baileys-antiban](https://github.com/kobie3717) by [@kobie3717](https://github.com/kobie3717) and has been bundled directly into phoenix-baileys-v2.

Anti-ban protection is currently **disabled by default**. Enable it explicitly via `makeWASocket({ antiban: ... })` if you want outbound messages to pass through the protection layers.

## How It Works

When anti-ban protection is enabled, every `sendMessage()` call passes through multiple protection layers:
...
```js
const sock = makeWASocket({ antiban: 'aggressive', auth: state })
...
## Default Usage (protection disabled)
...
```js
// Available only when anti-ban protection is enabled.
